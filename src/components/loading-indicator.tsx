...
const LoadingIndicator = ({ messageKey, className }: Props) => {
	const { t } = useTranslations();

	return (
		<>
			{messageKey ? <span className="sr-only">{t(messageKey)}</span> : null}
			<svg
				viewBox="0 0 32 32"
				version="1.1"
				xmlns="http://www.w3.org/2000/svg"
				className={cn('size-12 animate-spin fill-primary', className)}
			>
				<g strokeWidth="0" />
